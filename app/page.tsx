...
import { 
  ...
  Plus, Minus, Trash2, CheckCircle2, ShoppingCart, Loader2, Zap, ScanLine, AlertCircle, TrendingUp, User
} from 'lucide-react'
...
export default function ScanPage() {
  ...
  const handleSaveSingle = async () => {
    ...
    await supabase.from('transactions').insert([{
      ...
      old_stock: oldStock, new_stock: newStock, created_by: activeUser
    }]);
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#0a0f18] text-white overflow-hidden font-sans">
      ...
      <header className="px-6 py-4 flex justify-between items-center bg-[#1e293b] border-b border-white/5 z-[110]">
        ...
        <button onClick={() => router.push('/dashboard')} className="bg-white/5 px-4 py-2 rounded-full text-[10px] font-black uppercase border border-white/10">Admin Dashboard</button>
      </header>

      {/* 2. ส่วนเนื้อหา (Content) */}
      {!isScanning ? (
        /* 🌟 เมนูเลือกโหมด (จะโชว์ก็ต่อเมื่อยังไม่เปิดกล้อง) 🌟 */
          ...
          <div className="space-y-3">
             ...
             <div className="grid grid-cols-2 gap-4">
                <button onClick={() => startScanner('receive', 'single')} className="bg-slate-800 p-6 rounded-[2rem] border-b-4 border-slate-900 active:scale-95 flex flex-col items-center gap-3">
                   ...
                   <span className="text-[11px] font-black uppercase text-white leading-none text-center">แบบเดิม<br/><span className="text-slate-500 font-bold">(ทีละชิ้น)</span></span>
                </button>
                <button onClick={() => startScanner('receive', 'batch')} className="bg-green-600 p-6 rounded-[2rem] border-b-4 border-green-800 active:scale-95 flex flex-col items-center gap-3 shadow-lg shadow-green-900/20">
                   ...
                   <span className="text-[11px] font-black uppercase text-white leading-none text-center">แบบใหม่<br/><span className="text-green-100 font-bold">(สแกนต่อเนื่อง)</span></span>
                </button>
             </div>
          </div>
          ...
          <div className="space-y-3">
             ...
             <div className="grid grid-cols-2 gap-4">
                ...
                <button onClick={() => startScanner('issue', 'batch')} className="bg-red-600 p-6 rounded-[2rem] border-b-4 border-red-800 active:scale-95 flex flex-col items-center gap-3 shadow-lg shadow-red-900/20">
                   <Zap className="text-white animate-pulse" size={32}/>
                   <span className="text-[11px] font-black uppercase text-white leading-none text-center">แบบใหม่<br/><span className="text-red-100 font-bold">(สแกนต่อเนื่อง)</span></span>
                </button>
             </div>
          </div>
          ...
          {scanMethod === 'batch' && (
            <div className="flex-1 flex flex-col bg-slate-50 rounded-t-[2.5rem] mt-[-20px] z-10 shadow-2xl overflow-hidden">
               <div className="p-5 flex justify-between items-center border-b border-slate-200">
                  <span className="font-black uppercase text-xs italic text-slate-900">ในชุด ({basket.length})</span>
                  {basket.length > 0 && (
