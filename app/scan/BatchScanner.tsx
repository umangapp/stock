  ...
  const handleScan = async (sku: string) => {
    ...
    if (p) {
        if (scanMode === 'issue') {
            ...
            if ((inBasket ? inBasket.amount : 0) + 1 > p.current_stock) { 
                playErrorSound(); alert(`❌ สต๊อก "${p.name}" ไม่พอ!`); 
  ...
  return (
    <div className="flex-1 flex flex-col relative bg-black overflow-hidden font-sans">
      <div className="h-[300px] w-full relative bg-black shrink-0 border-b border-white/10">
        ...
        {visualLock && <div className="absolute inset-0 bg-green-500/30 flex items-center justify-center z-50 border-[10px] border-green-500 animate-in fade-in"><div className="bg-white px-8 py-3 rounded-full font-black text-green-600 shadow-2xl flex items-center gap-2"><CheckCircle2 size={20}/> SUCCESS!</div></div>}
      ...
      <div className="flex-1 flex flex-col bg-slate-50 rounded-t-[3rem] mt-[-30px] z-10 shadow-2xl overflow-hidden text-slate-900">
          <div className="p-6 flex justify-between items-center border-b bg-white shrink-0">
             <div className="flex items-center gap-2"><div className="bg-blue-100 p-2 rounded-xl text-blue-600"><ShoppingCart size={20}/></div><span className="font-black uppercase text-sm italic tracking-tight">ในชุดสแกน ({basket.length})</span></div>
                ...
                <div key={item.sku_15_digits} className={`bg-white p-5 rounded-[2.5rem] border shadow-sm flex justify-between items-center shrink-0 transition-all duration-500 ${index === 0 ? 'border-blue-400 ring-2 ring-blue-500/10 scale-[1.02] z-20 shadow-blue-500/10' : 'border-slate-100'}`}>
                   <div className="flex-1 pr-4">
                      ...
                      <div className="flex items-center gap-2 mt-1.5 leading-none">
                         ...
                         <span className="text-[9px] font-black text-slate-500 uppercase bg-slate-100 px-1.5 py-0.5 rounded border leading-none">LOT: {item.received_date}</span>
                      </div>
                   </div>
                   <div className="flex items-center gap-2">
                      <div className="flex items-center bg-slate-100 p-1.5 rounded-2xl gap-3 border border-slate-200">
                         ...
                         <button onClick={() => {
                             ...
                             if (scanMode === 'issue' && newQty > item.current_stock) { playErrorSound(); alert("สต๊อกไม่พอ!"); return; }
          ...
          <div className="fixed inset-0 bg-slate-900/98 backdrop-blur-2xl z-[500] flex items-center justify-center p-4">
              <div className="bg-white w-full max-w-md rounded-[3rem] p-8 flex flex-col max-h-[90vh] animate-in zoom-in duration-300">
                  <div className="flex flex-col items-center text-center mb-6 shrink-0">
                     ...
                     <p className="text-[10px] font-bold text-slate-400 mt-2 uppercase tracking-widest">ตรวจสอบข้อมูล {basket.length} รายการก่อนบันทึก</p>
                  </div>
                  <div className="flex-1 overflow-y-auto space-y-3 mb-6 pr-2">
                      {basket.map(item => (
                          <div key={item.id} className="bg-slate-50 p-4 rounded-3xl border border-slate-100 flex justify-between items-center shadow-sm">
                              <div className="flex-1 pr-4">
                  ...
                  <div className="flex gap-4 shrink-0">
                      <button onClick={() => setShowBatchSummary(false)} className="flex-1 py-5 rounded-3xl font-black text-slate-400 bg-slate-100 uppercase text-xs active:scale-95 transition-all">กลับไปแก้</button>
                      <button onClick={handleFinalSave} disabled={isSaving} className={`flex-[2] py-5 rounded-3xl font-black text-white shadow-xl ${scanMode === 'receive' ? 'bg-green-600' : 'bg-red-600'} disabled:opacity-50 active:scale-95 transition-all flex items-center justify-center gap-2`}>
