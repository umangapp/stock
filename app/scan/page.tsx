...
export default function ScanPage() {
  ...
  const handleSaveTransaction = async () => {
    ...
    if (isIssue && amount > oldStock) {
      alert(`❌ สต๊อกไม่พอ! มีแค่ ${oldStock}`);
      return;
    }
          ...
          <div className="bg-white w-full max-w-md rounded-t-[3rem] sm:rounded-[3rem] p-8 text-slate-900 shadow-2xl">
            <div className="flex justify-between items-start mb-6">
              <div className="flex-1">
                ...
                <p className="text-blue-600 font-mono font-black text-sm tracking-widest mb-1 italic">{selectedProduct.sku_15_digits}</p>
                
                {/* 🌟 ลำดับใหม่: หนา x กว้าง x ยาว (เช่น 3.2 x 10 x 2000) */}
                ...
                <p className="text-[11px] font-black text-slate-400 mt-2 uppercase tracking-widest italic">Lot Date: {selectedProduct.received_date}</p>
              </div>
              ...
              <div className="space-y-3 mb-6">
                 <div className="flex flex-col border-b pb-2">
                    ...
                    <div className="text-right mt-1">
                       <p className="text-[11px] font-black text-blue-600 font-mono tracking-tighter italic">{selectedProduct.sku_15_digits}</p>
                       
                       {/* 🌟 ลำดับใหม่: หนา x กว้าง x ยาว (เช่น 3.2 x 10 x 2000) */}
                       <p className="text-[10px] font-bold text-slate-500 italic mt-0.5">
                         {selectedProduct.height} x {selectedProduct.width} x {selectedProduct.length} มม.
                       </p>
