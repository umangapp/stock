...
import { 
  ...
  ArrowDownCircle, ArrowUpCircle, Plus, Minus, FileText, AlertCircle, TrendingUp, User
} from 'lucide-react'
...
export default function ScanPage() {
  ...
  const handleSaveTransaction = async () => {
    ...
    if (isIssue && amount > oldStock) {
      alert(`❌ สต๊อกไม่พอ! \nมีอยู่แค่ ${oldStock} แต่พยายามนำออก ${amount}`);
      return;
    }
  ...
  if (loading) return <div className="h-screen bg-[#0a0f18] flex items-center justify-center text-blue-500 font-black italic tracking-widest">LOADING...</div>
                  ...
                  <div key={date} className="space-y-4">
                    ...
                    {logs.map((log: any) => (
                      <div key={log.id} className="bg-white rounded-[1.5rem] p-5 flex flex-col gap-2 shadow-xl">
                        ...
                        {/* 🌟 จุดที่แก้ไข: เพิ่มชื่อ User Login ตรงส่วนท้ายของบัตรประวัติ */}
                        <div className="flex justify-between items-center border-t border-slate-50 pt-2">
                           <div className="flex items-center gap-1.5">
                              <User size={12} className="text-blue-500" />
                              <span className="text-[10px] font-black uppercase text-slate-600">{log.created_by}</span>
                           </div>
                           <div className="flex items-center gap-1 text-[10px] font-bold text-slate-300 italic">
                              <Clock size={12} /> {new Date(log.created_at).toLocaleTimeString('th-TH')}
                           </div>
                        </div>
                      </div>
                    ))}
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[300] flex items-end sm:items-center justify-center">
          <div className="bg-white w-full max-w-md rounded-t-[3rem] sm:rounded-[3rem] p-8 text-slate-900 animate-in slide-in-from-bottom duration-300">
            ...
            <div className="space-y-6">
              <div className="bg-slate-900 text-white p-4 rounded-2xl flex justify-between items-center"><span className="text-[10px] font-black uppercase text-slate-400">สต๊อกเดิม</span><span className="text-xl font-black">{selectedProduct.current_stock} {selectedProduct.unit}</span></div>
              ...
              <button onClick={() => setShowSummaryModal(true)} className={`w-full py-6 rounded-[2rem] font-black text-xl uppercase italic text-white ${scanMode === 'receive' ? 'bg-green-600 shadow-green-200' : 'bg-red-600 shadow-red-200'}`}>ยืนยันรายการ</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/98 backdrop-blur-xl z-[400] flex items-center justify-center p-4">
           <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-6 text-slate-900 animate-in zoom-in duration-200">
              ...
              <div className="flex gap-3">
                 ...
                 <button onClick={handleSaveTransaction} className={`flex-[2] py-3.5 rounded-xl font-black text-white shadow-lg ${scanMode === 'receive' ? 'bg-green-600' : 'bg-red-600'}`}>ยืนยันบันทึก</button>
              </div>
           </div>
        </div>
      )}
