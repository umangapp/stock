...
import { QrCode, Clock, History, Calendar, X, Camera, Search, ArrowDownCircle, ArrowUpCircle, Plus, Minus, FileText, AlertCircle, TrendingUp } from 'lucide-react'
...
export default function ScanPage() {
  ...
  const fetchUserData = useCallback(async () => {
    ...
    const currentName = profile?.full_name || session.user.email?.split('@')[0];
  ...
  const handleSaveTransaction = async () => {
    ...
    if (!updateErr && !logErr) { setShowSummaryModal(false); setShowActionModal(false); fetchUserData(); setScanInput(''); }
  }
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#0a0f18] text-white overflow-hidden">
      ...
      <header className="px-6 py-5 flex justify-between items-center bg-[#1e293b] border-b border-white/5">
        ...
        <button onClick={() => setShowHistory(true)} className="bg-blue-600 px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-widest active:scale-95 transition-all">ประวัติรายการ</button>
      </header>

      <div className="flex-1 relative flex flex-col">
        {/* HISTORY OVERLAY (Math History) */}
        {showHistory && (
          <div className="absolute inset-0 bg-[#0a0f18] z-[120] p-6 overflow-y-auto animate-in slide-in-from-right duration-300">
             <div className="max-w-md mx-auto space-y-6 pb-20 text-slate-900">
                <div className="flex justify-between items-center text-white mb-8"><h3 className="text-2xl font-black italic uppercase text-blue-500">History</h3><button onClick={() => setShowHistory(false)} className="bg-white/10 p-2 rounded-full"><X/></button></div>
                {personalLogs.map((log: any) => (
                  <div key={log.id} className="bg-white rounded-3xl p-5 flex flex-col gap-2 shadow-xl">
                    <div className="flex justify-between">
                      <p className="font-black text-lg">{log.products?.name}</p>
                      <span className={`font-black text-2xl ${log.type === 'receive' ? 'text-green-600' : 'text-red-600'}`}>{log.type === 'receive' ? '+' : '-'} {log.amount}</span>
                    </div>
                    {/* 🌟 แสดงคณิตศาสตร์สต๊อกในประวัติ */}
                    <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl border">
                       <TrendingUp size={12} className="text-blue-500" />
                       <p className="text-[11px] font-black text-slate-600 uppercase">สต๊อก: {log.old_stock || 0} {log.type === 'receive' ? '+' : '-'} {log.amount} = {log.new_stock || 0}</p>
                    </div>
                    <p className="font-mono text-[10px] text-blue-400">{log.products?.sku_15_digits}</p>
                  </div>
                ))}
             </div>
          </div>
      ...
      {/* ACTION MODAL (±5, ±10, ±50) */}
      {showActionModal && selectedProduct && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[300] flex items-end sm:items-center justify-center">
          <div className="bg-white w-full max-w-md rounded-t-[3rem] sm:rounded-[3rem] p-8 text-slate-900">
            <div className="flex justify-between items-start mb-6">
              <div><h2 className="text-2xl font-black uppercase italic leading-none">{selectedProduct.name}</h2><p className="text-[12px] font-black text-slate-400 mt-2 uppercase">ขนาด: {selectedProduct.width} x {selectedProduct.height} x {selectedProduct.length} มม.</p></div>
              <button onClick={() => setShowActionModal(false)} className="bg-slate-100 p-2 rounded-full text-slate-400"><X/></button>
            </div>
            <div className="space-y-6">
              <div className="bg-slate-900 text-white p-4 rounded-2xl flex justify-between items-center"><span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">สต๊อกเดิม</span><span className="text-xl font-black">{selectedProduct.current_stock} {selectedProduct.unit}</span></div>
              <div className="grid grid-cols-3 gap-3">
                {[5, 10, 50].map(n => (
                  <button key={n} onClick={() => setAmount(prev => prev + n)} className={`${scanMode === 'receive' ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-red-50 text-red-600 border-red-200'} py-4 rounded-2xl font-black text-xl border-b-4 active:translate-y-1 active:border-b-0 transition-all`}>{scanMode === 'receive' ? '+' : '-'}{n}</button>
                ))}
              </div>
              ...
              <div className="relative"><FileText className="absolute left-4 top-4 text-slate-300" size={20}/><textarea placeholder="หมายเหตุ..." className="w-full bg-slate-50 border rounded-2xl p-4 pl-12 text-sm font-bold h-20" value={note} onChange={(e) => setNote(e.target.value)}/></div>
              <button onClick={() => setShowSummaryModal(true)} className={`w-full py-6 rounded-[2rem] font-black text-xl uppercase italic text-white ${scanMode === 'receive' ? 'bg-green-600' : 'bg-red-600'}`}>ยืนยัน{scanMode === 'receive' ? 'นำเข้า' : 'นำออก'}</button>
            </div>
          </div>
        </div>
      )}

      {/* --- 🌟 SUMMARY MODAL (NO SCROLL + SIZE + MATH) --- */}
           ...
           <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-6 text-slate-900 shadow-2xl animate-in zoom-in duration-200">
              ...
              <div className="space-y-3 mb-6">
                 <div className="flex flex-col border-b border-slate-100 pb-2">
                    ...
                    <div className="text-right mt-1"><span className="text-[10px] font-bold text-blue-500 italic">({selectedProduct.width} x {selectedProduct.height} x {selectedProduct.length} มม.)</span></div>
                 </div>
                 ...
                 <div className="bg-slate-50 p-3 rounded-xl">
                    ...
                    {/* 🌟 แสดงคำนวณเลขในหน้าสรุป */}
                    <div className="flex justify-between text-[14px] font-black mt-1 pt-1 border-t border-slate-200 italic">
                       <span>สต๊อกใหม่:</span>
