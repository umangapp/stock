...
export default function ScanPage() {
  ...
  const handleSaveTransaction = async () => {
    ...
    if (!updateErr && !logErr) {
      ...
      alert("❌ เกิดข้อผิดพลาด: " + (updateErr?.message || "ไม่สามารถบันทึกได้"));
    }
  }
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#0a0f18] text-white overflow-hidden">
      <header className="px-6 py-5 flex justify-between items-center bg-[#1e293b] border-b border-white/5">
        <div className="flex items-center gap-3"><QrCode className="text-blue-500" /><h1 className="text-xs font-black uppercase italic text-blue-400 leading-none">Scan Center</h1></div>
      ...
      {showSummaryModal && selectedProduct && (
        <div className="fixed inset-0 bg-slate-900/98 backdrop-blur-xl z-[400] flex items-center justify-center p-4">
           <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-6 text-slate-900 animate-in zoom-in duration-200">
