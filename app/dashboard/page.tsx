...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    const { data: p } = await supabase.from('products').select('*')
      .order('height', { ascending: true }) // เรียงจากหนาน้อยไปมาก
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 lg:flex-row overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto p-4 lg:p-10 pb-24 text-slate-900">
        {activeTab === 'dashboard' && (
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-slate-800">
             <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
               ...
               <div className="flex flex-col sm:flex-row gap-3 mb-6">
                 ...
                 <button onClick={() => { supabase.from('settings_product_master').insert([{name: inputName, prefix: inputPrefix}]).then(() => {setInputName(''); setInputPrefix(''); fetchData();}) }} className="bg-blue-600 text-white p-4 rounded-2xl shadow-lg hover:bg-blue-700 transition-all active:scale-95"><Plus/></button>
               </div>
               <div className="space-y-2 max-h-96 overflow-y-auto pr-2">
                 {masterProducts.map(item => (
                   <div key={item.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 font-bold shadow-sm">
             ...
             <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
               ...
               <div className="space-y-2 max-h-96 overflow-y-auto pr-2">
                 {masterUnits.map(item => (
                   <div key={item.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 font-black shadow-sm">
                     <span>{item.unit}</span>
          ...
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl p-8 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={isAddModalOpen ? handleAddProduct : handleUpdateProduct} className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                {/* 🌟 เรียงลำดับ Field ใหม่ตามที่สั่ง: หนา (Height) -> กว้าง (Width) -> ยาว (Length) */}
