...
import { 
  ...
  Search, ChevronDown, ChevronUp, Edit3, X, Save, LogOut, Plus, QrCode, TrendingUp, User
} from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  // --- 1. ฟังก์ชันสร้าง SKU 15 หลัก (เติม x ต่อท้าย) ---
  const generateSKU = (p: any) => {
    ...
    // เติม x ต่อท้ายให้ครบ 15 หลัก (ไม่มีขีด)
    return raw.padEnd(15, 'x');
  }
        ...
        {activeTab === 'inventory' && (
          <div className="space-y-8 animate-in fade-in">
            <div className="flex flex-col md:flex-row justify-between items-end gap-4">
              <div><h2 className="text-3xl font-black uppercase italic tracking-tighter text-slate-900 leading-none">คลังสินค้า</h2><p className="text-[10px] text-slate-400 font-bold uppercase mt-2">จัดการสต๊อก (SKU Padding 'x')</p></div>
        ...
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-[4rem] shadow-2xl p-10 animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={isAddModalOpen ? handleAddManual : async (e) => {
              ...
              <button type="submit" className="w-full bg-blue-600 text-white py-6 rounded-[2.5rem] font-black shadow-xl shadow-blue-200 uppercase tracking-tighter italic hover:bg-blue-700 transition-all active:scale-95">
