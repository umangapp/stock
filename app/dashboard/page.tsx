...
export default function AdminDashboard() {
  ...
  const addMaster = async (table: string, data: any) => {
    ...
    else { setInputName(''); setInputPrefix(''); setInputUnit(''); fetchData(); }
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 lg:flex-row overflow-hidden font-sans text-slate-900">
      ...
      <nav className="w-full lg:w-72 bg-slate-900 text-white p-6 flex lg:flex-col gap-2 shrink-0 z-20">
        ...
        <button onClick={handleLogout} className="flex items-center gap-4 px-6 py-4 rounded-3xl text-sm font-bold text-red-400 hover:bg-red-500/10 mt-auto"><LogOut size={20}/> ออกจากระบบ</button>
      </nav>
          ...
          <div className="space-y-8 animate-in fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Object.entries(groupedByUser).map(([user, logs]: [string, any]) => (
                <div key={user} className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden h-fit transition-all hover:shadow-xl">
                      ...
                      {logs.map((log: any) => (
                        <div key={log.id} className="bg-white p-5 rounded-3xl border shadow-sm flex flex-col gap-3">
                          ...
                          <div className="flex justify-between items-center text-[10px] font-black text-slate-300 uppercase italic border-t pt-2">
                             ...
                             <span className="flex items-center gap-1"><Clock size={12}/> {new Date(log.created_at).toLocaleTimeString('th-TH')}</span>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <div className="space-y-8 animate-in fade-in">
            <div className="flex flex-col md:flex-row justify-between items-end gap-4">
               <div>
                  ...
                  <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Inventory Management</p>
               </div>
              ...
              {Object.values(groupedInventory).filter((g: any) => g.name.toLowerCase().includes(searchQuery.toLowerCase())).map((group: any) => (
                <div key={group.name} className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden h-fit transition-all hover:border-blue-400">
                  <div onClick={() => setExpandedGroups(prev => prev.includes(group.name) ? prev.filter(n => n !== group.name) : [...prev, group.name])} className="p-7 cursor-pointer hover:bg-slate-50 transition-all flex justify-between items-center text-slate-800">
                    <div>
                        <h3 className="font-black uppercase text-xl truncate w-32 tracking-tighter">{group.name}</h3>
            ...
            <form onSubmit={isAddModalOpen ? handleAddProduct : handleUpdateProduct} className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div className="col-span-full">
                  ...
                  <select required className="w-full bg-slate-50 p-4 rounded-2xl outline-none font-bold border focus:border-blue-500" value={isAddModalOpen ? newProduct.name : editingProduct.name} onChange={e => handleNameSelect(e.target.value, isAddModalOpen)}>
