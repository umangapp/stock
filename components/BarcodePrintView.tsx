...
export default function BarcodePrintView({ products }: BarcodePrintViewProps) {
  ...
  // แผงควบคุมส่วนกลาง (เหลือแค่ชื่อบริษัทและโลโก้)
  const [companyName, setCompanyName] = useState('บริษัท อุมัง จำกัด')
  const [logoUrl, setLogoUrl] = useState<string | null>(null)

  // 🔒 เก็บสถานะตัวเลือกสินค้าแต่ละแถว (เพิ่มโควตา showAmount แยกชิ้นตามสั่งของพี่ตั้ม)
      ...
      if (setting.checked && setting.copies > 0) {
        for (let i = 0; i < setting.copies; i++) {
          cards.push({
            ...
            showAmount: setting.showAmount // ส่งสถานะแยกชิ้นไปที่ตัวสติกเกอร์
          })
        }
      }
    })
  ...
  return (
    <div className="space-y-8 animate-in fade-in">
      ...
      {/* แผงควบคุมตั้งค่าส่วนกลาง (หัวบริษัท, โลโก้) */}
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6 no-print text-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-100 pt-6 items-end">
          <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 no-print text-slate-800">
        <div className="xl:col-span-2 bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col max-h-[700px]">
          ...
          {/* รายการตารางสินค้าคัดเลือกหลังบ้าน */}
            ...
            {filteredProducts.map(item => {
              ...
              return (
                <div key={item.id} className={`p-4 rounded-2xl border transition-all flex flex-col lg:flex-row items-start lg:items-center gap-4 ${state.checked ? 'bg-blue-50/40 border-blue-400 shadow-sm' : 'bg-slate-50/50 border-slate-100'}`}>
                  ...
                  {/* 🔒 โซนกล่องเครื่องมือป้อนข้อมูล จัดระเบียบใหม่ให้ขยายตัวเห็นชัดเจนไม่โดนบัง */}
                  <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto lg:ml-auto pt-3 lg:pt-0 border-t lg:border-t-0 border-slate-200">
                    ...
                    {/* ช่องคีย์จำนวนสินค้าสลักป้าย (จะโผล่มาเฉพาะเมื่อติ๊กเลือกเปิดแสดงจำนวนรายตัว) */}
                    {state.showAmount && (
                      <div>
                    ...
                    {/* 2. 💥 กล่องสีส้มไฮไลต์เด่นชัด แก้ปัญหารอบก่อนที่มองไม่เห็นจุดกรอกจำนวนใบสติ๊กเกอร์ */}
                    <div className="bg-amber-50 border border-amber-300 p-1.5 rounded-xl text-center shadow-inner">
                      ...
                      <input 
                        type="number" 
                        min={1} 
                        className="w-20 bg-white border border-amber-400 h-[32px] p-1 rounded-lg text-sm font-black text-center text-amber-900 focus:border-amber-600 outline-none" 
                        value={state.copies} 
                        onChange={e => updateItemSetting(item.id, 'copies', Math.max(1, parseInt(e.target.value) || 1))} 
                      />
                    </div>
                  </div>
        ...
        <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white flex flex-col h-[700px] shadow-2xl">
          ...
          <div className="flex-1 flex items-center justify-center bg-white/5 rounded-3xl p-4 overflow-hidden">
            {printCards.length > 0 ? (
              /* 3. 🎯 ปรับปรุงเพิ่มขนาด Font ของตัวป้ายสติ๊กเกอร์ขึ้น 1 Step เพื่อให้อ่านง่าย */
              <div className="bg-white text-slate-900 p-5 rounded-2xl w-full max-w-[270px] shadow-lg flex flex-col justify-between border border-slate-200" style={{ minHeight: '350px' }}>
                <div className="space-y-2 text-[14px] font-bold text-slate-900 leading-normal">
                  <p className="text-center font-black border-b pb-1 text-blue-600 uppercase tracking-tight text-sm">{companyName}</p>
                  ...
                  {/* แสดงผลจำนวนเฉพาะใบที่โดนสั่งติ๊กเลือกเปิดใช้งานเท่านั้น */}
                ...
                <div className="mt-4 flex flex-col items-center gap-1 bg-slate-50 p-3 rounded-xl border border-slate-200">
                  <QRCodeCanvas { /* @ts-ignore */ ...{}} value={printCards[0].sku_15_digits} size={110} level="H" imageSettings={logoUrl ? { src: logoUrl, height: 22, width: 22, excavate: true } : undefined} />
                  <span className="font-mono font-black text-[11px] tracking-widest mt-1 text-slate-700">{printCards[0].sku_15_digits}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {printCards.map((card, idx) => (
                  <div key={idx} className="border border-slate-300 rounded-lg p-3 flex flex-col justify-between bg-white text-slate-900" style={{ height: '67mm', boxSizing: 'border-box' }}>
                    <div className="space-y-1 text-[13px] font-bold leading-normal text-slate-900">
                    ...
                    <div className="flex flex-col items-center mt-1 bg-slate-50 p-1.5 rounded border border-slate-100">
                      <QRCodeCanvas { /* @ts-ignore */ ...{}} value={card.sku_15_digits} size={80} level="H" imageSettings={logoUrl ? { src: logoUrl, height: 16, width: 16, excavate: true } : undefined} />
                      <span className="font-mono font-black text-[10px] tracking-wider mt-0.5 text-slate-800">{card.sku_15_digits}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* 🔒 🖨️ โครงสร้าง HTML แท้ๆ ขยายขนาดอักษร 1 Step เต็มสเปกเพื่อยิงเข้าเครื่องพิมพ์ / PDF ใบจริง */}
          ...
          <div key={pageIdx} className="sticker-page">
            {printCards.slice(pageIdx * 12, (pageIdx + 1) * 12).map((card, idx) => (
              <div key={idx} className="sticker-card">
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '14px', fontWeight: 'bold', lineHeight: '1.3', color: '#000000' }}>
                  <p style={{ textAlign: 'center', fontWeight: '900', borderBottom: '1px solid #cbd5e1', paddingBottom: '4px', color: '#2563eb', textTransform: 'uppercase', fontSize: '15px' }}>{companyName}</p>
                ...
                <div style={{ marginTop: '4px', display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: '#f8fafc', padding: '5px', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
                  <QRCodeCanvas { /* @ts-ignore */ ...{}} value={card.sku_15_digits} size={90} level="H" imageSettings={logoUrl ? { src: logoUrl, height: 18, width: 18, excavate: true } : undefined} />
                  <span style={{ fontFamily: 'monospace', fontWeight: '900', fontSize: '11px', letterSpacing: '0.1em', marginTop: '3px', color: '#000000' }}>{card.sku_15_digits}</span>
                </div>
              </div>
            ))}
          </div>
